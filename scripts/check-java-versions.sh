...
# reports the bytecode target for visibility, except maven.compiler.release which is
# also checked against the Dockerfile's runtime image below.
for prop in maven.compiler.release maven.compiler.source maven.compiler.target; do
  ...
  echo "ℹ️  Upstream targets Java $value bytecode (<$prop>)"
  if [ "$prop" = "maven.compiler.release" ]; then
    BYTECODE_TARGET="$value"
  fi
done

DOCKERFILE_JAVA_VERSION=$(sed -nE 's/^FROM +[^:[:space:]]+:([0-9]+).*/\1/p' "$DOCKERFILE_PATH" | head -n 1)
if [ -z "$DOCKERFILE_JAVA_VERSION" ]; then
  echo "❌ Could not parse a Java version from $DOCKERFILE_PATH's base image tag — its FROM line may have changed format." >&2
  exit 1
fi

if [ "$DOCKERFILE_JAVA_VERSION" != "$BYTECODE_TARGET" ]; then
  echo "❌ Dockerfile runtime image is Java $DOCKERFILE_JAVA_VERSION but upstream's bytecode target (maven.compiler.release) is Java $BYTECODE_TARGET — update the Dockerfile base image." >&2
  exit 1
fi
echo "✅ Dockerfile runtime (Java $DOCKERFILE_JAVA_VERSION) matches upstream's bytecode target (maven.compiler.release $BYTECODE_TARGET)"
