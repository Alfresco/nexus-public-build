...
- **Java**: build JDK pinned in [`.java-version`](.java-version) (currently 25), Temurin recommended. This is independent of upstream's `maven.compiler.release` (currently 21), which pins the bytecode target the build produces — Maven's own `maven-enforcer-plugin` already fails the build if the build JDK doesn't meet upstream's minimum. `scripts/check-java-versions.sh` reports the bytecode target and fails if your local JDK doesn't match the pin.
